...
const FoodCard = React.forwardRef<HTMLDivElement, FoodCardProps>(({ food, reorderable }, ref) => {
  ...
  const cardContent = (
    ...
    >
      <CardHeader className="p-0 relative">
        ...
        />
         {reorderable && (
            <div className="absolute top-2 right-2 p-1 bg-gray-900/50 rounded-full cursor-grab active:cursor-grabbing touch-none"
...
function SortableFoodCard({ food }: { food: Food }) {
  ...
  return (
    <div ref={setNodeRef} style={style} {...attributes} {...listeners}>
       <FoodCard food={food} reorderable={true} />
    </div>
  );
}

export default function FoodCardWrapper({ food, reorderable }: FoodCardProps) {
  if (reorderable) {
